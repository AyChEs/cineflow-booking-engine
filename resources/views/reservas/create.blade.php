...
<div class="container" style="max-width: 600px; padding-top: 3rem;">
    <div style="background: var(--color-bg-secondary); border: 1px solid var(--border-subtle); border-radius: 12px; padding: 2rem;">
        <h1 class="page-title" style="margin-bottom: 1.5rem;">NEW BOOKING</h1>
        ...
        <form method="POST" action="{{ route('reservas.store') }}">
            ...
            <div class="form-group">
                ...
                <select id="fk_usuario_id" name="fk_usuario_id" class="form-select" required>
                    ...
                    @foreach ($usuarios as $user)
                        <option value="{{ $user->id }}" {{ old('fk_usuario_id') == $user->id ? 'selected' : '' }}>
            ...
            <div class="form-group">
                ...
                <input type="text" id="butaques_seleccionades" name="butaques_seleccionades" class="form-input"
                       placeholder="E.g. A1, A2, B5" value="{{ old('butaques_seleccionades') }}" required>
            ...
            <div class="form-group">
                ...
                <input type="number" id="total_pagat" name="total_pagat" class="form-input"
                       step="0.01" min="0" value="{{ old('total_pagat') }}" required>
            ...
            <div class="form-group">
                ...
                <select id="estat" name="estat" class="form-select" required>
                    ...
                    <option value="pendent"  {{ old('estat') === 'pendent'  ? 'selected' : '' }}>Pending</option>
                    <option value="pagat"    {{ old('estat') === 'pagat'    ? 'selected' : '' }}>Paid</option>
                    <option value="cancelat" {{ old('estat') === 'cancelat' ? 'selected' : '' }}>Cancelled</option>
                </select>
            ...
            <div style="display: flex; gap: 1rem; margin-top: 1.5rem;">
                ...
                <a href="{{ route('reservas.index') }}" class="btn btn-secondary">Cancel</a>
            </div>
        </form>
    </div>
</div>
